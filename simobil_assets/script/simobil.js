    ...
    $(function() {
        $('.lightbox-trigger').click(function(e) {
            ...
            if (_opened_lightbox != null) {
                ...
                var opened= $($(_opened_lightbox).attr("rel"));
                console.log(opened);
                opened.html(toOpen.html());*/
                //$(_opened_lightbox).overlay().close();
            }
        });
        $('.large-table').on('click', '[rel="tooltip"]', function(e){
            e.preventDefault();
        });
        ...
        $('nav ul.first').on('click', 'li.has-sub-menu', function(e) {
            ...
            if ($(e.target).parents('.submenu').length == 0) {
                ...
                $('body').toggleClass('main-menu-opened', $('li.has-sub-menu.active').length > 0);
                return false;
            }
        });
        ...
        $('.telefonske-stevilke').on('keydown', 'input', function(event) {
            ...
            if (event.keyCode == '13') {
                if (value.length > 0) {
                    ...
                    input.val('').prev('.name').removeClass('edit').html('Dodajte opis');
                }
                return false;
            }
        ...
        $('.telefonske-stevilke').on('blur', 'input', function(e){
            var input = $(this),
                value = input.val();
        ...
        if ($('.eracun-bottom').length) {

            var EB = $('.eracun-bottom');
            ...
            EB.on('click', '.close-eracun-bottom', function() {
                EB.addClass('closed');
            });

        }

        if ($('.social-login-bottom').length) {

            var SLB = $('.social-login-bottom');

            /** Animate social login on page load */
            $(window).on('load', function() {
                setTimeout(function() {
                    SLB.addClass('opened');
                }, 1000);
            ...
            SLB.on('click', '.close-social-login-bottom', function() {
                SLB.addClass('closed');
            });
        ...
        /** Active state pri poslovnih centrih */
        $('.poslovni-centri .locations-list').on('click', '.item', function() {
            var poslovniCentri = $('.poslovni-centri .locations-list'),
                that = $(this);

            poslovniCentri.find('.item.active').removeClass('active');

            that.hasClass('item') ? that.addClass('active') : that.parents('.item').addClass('active');
        });
        ...
        /** Sestavite si paket */
        if ($('.sestavite-si-paket').length) {

            var SIP = $('.sestavite-si-paket'),
                totalValue = 0;

            /** Adding options */
            SIP.on('click', '.first li:not(.heading)', function(event) {
                var that = $(this),
                    thatID = that.attr('id');

                /** If we click on input, dont toggle the LI element */
                ...
                if (!that.hasClass('active')) {
                    ...
                    var clone = that.clone().appendTo(SIP.find('.second'));
                    ...
                    SIP.find('.second #' + thatID).remove();
                }

                /** Initializing tooltips */
                SIP.find('.first input, .second input').tooltip({
                    title: 'Količina'
                });

                SIP.find('.value').first().html(calculatePaketPrice());
            });

            /** Removing options */
            SIP.on('click', '.second .ico-close-blue', function() {
                var option = $(this).parents('li'),
                    thatID = option.attr('id');

                SIP.find('.first #' + thatID).removeClass('active');
                option.remove();

                SIP.find('.value').first().html(calculatePaketPrice());
            });

            SIP.on('change keyup keypress', '.first input, .second input', function(event) {

                event.charCode === 13 ? $(this).blur() : '';
            ...
            function calculatePaketPrice() {
                totalValue = 0;
                var totalQuantity = 0,
                    normaOffer = SIP.find('.normal-offer'),
                    specialOffer = SIP.find('.special-offer'),
                    location = SIP.find('.second').is(':visible') ? 'second' : 'first';

                SIP.find('.' + location + ' li.active:not(.heading)').each(function() {
                    var that = $(this),
                        price = parseFloat(that.data('price')),
                        quantity = parseInt(that.find('input').val()) || 1;

                    totalValue += (price * quantity);
                    totalQuantity += quantity;
                });

                updateSelectedPackagesHiddenInputFields();

                if (totalQuantity < 50) {
                    specialOffer.removeClass('active');
                    normaOffer.addClass('active');
                    return (totalValue.toFixed(2).replace('.', ',')).replace(/,(.[^€])/g, ',<sup>$1</sup> €');
                } else {
                    normaOffer.removeClass('active');
                    specialOffer.addClass('active');
                }
            }

            function updateSelectedPackagesHiddenInputFields() {
                var SelectedPackates = '',
                    AllPackages = SIP.find('.second li'),
                    NumberOfPackages = AllPackages.length,
                    InputField = SIP.find('#selected-packages'),
                    i = 0;

                AllPackages.each(function() {

                    var that = $(this),
                        name = that.children('span').html(),
                        quantity = that.find('input').val();

                    SelectedPackates += i === NumberOfPackages - 1 ? quantity + 'x ' + name : quantity + 'x ' + name + ', ';

                    i++;

                });

                InputField.val(SelectedPackates);
        ...
        /** Rezervacija za webinarje */
        $('body').on('click', '.btn-prijava-na-event', function(e) {
            e.preventDefault();
        ...
        $('.configurator').on('change', '.choice input[type=checkbox], .shops input[type=checkbox]', function() {
            var _el = $(this);
        ...
        $('.configurator').on('click', '.choice .c4 a', function() {
        ...
        */
        ...
        function closeOthers() {
            $('.collapse.in:not(.stop)').collapse('hide');
        }
        ...
        function equalize_team_list() {
            if (screen.width > 767) {
                setTimeout(function() {
                    Simobil.equalize('.corpo .equal-height', '.item');
                }, 200);
            }
        }
        ...
        /** Discounts for comparison table */
        if ($('.comparison-table').length) {
            var CT = $('.comparison-table');

            /** Wrap table in wrapper, make it responsive */
            if( CT.width() > $('.container').width() && $(window).width() > 768) {
                CT.wrap('<div class="table-wrapper"></div>');
            }

            /** Table has discounts */
            if (CT.hasClass('has-discounts')) {
                var allPackages = CT.find('thead tr:first-child th'),
                    DI = CT.find('.discount-inner:first-child'),
                    numberOfPackages = allPackages.length - 1, // First column is in sidebar, so it doesnt count
                    packagesWithDiscount = [],
                    i = 1;

                allPackages.not(':eq(0)').each(function() {
                    var column = $(this);

                    if (column.hasClass('has-discount')) {
                        packagesWithDiscount.push(i);
                    }

                    i++;
                });

                /** If all packages has discounts, dont show the triangles */
                if (packagesWithDiscount.length === numberOfPackages) return;

                /** If only one package exists, append one triangle at the center */
                if (numberOfPackages === 1) {
                    /** @type {jquery} triangle element */
                    var triangle = $('<span>', {
                        'class': 'triangle'
                    });
                    DI.append(triangle);
                    triangle.css('left', 'calc(50% - 12px)');

                } else {
                    /** Show all the triangles at their positions */
                    for (var j = 1; j <= packagesWithDiscount.length; j++) {

                        var currentColumnCenterOffset = $(allPackages[packagesWithDiscount[j - 1]]).position().left + ($(allPackages[packagesWithDiscount[j - 1]]).width() / 2) - $(allPackages).eq(0).width() - 18; // 12 is for column padding, 10 is for triangle half width, together is 22
                        ...
                        DI.append(triangle);
                        triangle.css('left', currentColumnCenterOffset)
                    }
                }
        ...
        $('.eracun .infogram li, .landing-promo-page .infogram li, .why-simobil .infogram li, .ponudba-za-velika-podjetja .infogram li, .ponudba-za-mala-podjetja .infogram li').on('mouseover', function(e) {

            if (!$(this).children('a').length) return;

            $('.eracun .infogram li, .landing-promo-page .infogram li, .why-simobil .infogram li, .ponudba-za-velika-podjetja .infogram li, .ponudba-za-mala-podjetja .infogram li').removeClass('active');
            $(this).addClass('active');

            $_pane = $('.eracun .infogram .pane, .landing-promo-page .infogram .pane, .why-simobil .infogram .pane, .ponudba-za-velika-podjetja .infogram .pane, .ponudba-za-mala-podjetja .infogram .pane');
        ...
        $('.moj-simobil').on('change', '.paket-selector-table input[type="radio"]', function() {
            var that = $(this),
                        ...
                        setTimeout(function() {
                            ...
                            if ($(window).width() > 767) {
                                $('.c3, ', $data).css({
                                    ...
                                    'line-height': $data.innerHeight() + "px"
                                });
                                $('.c3, ', $data).parent().parent().find(".mini-info").css('line-height', $data.innerHeight() + "px");
                                $('.c3, ', $data).parent().parent().find(".c12").find("label").css({
                                    'height': $data.innerHeight(),
                                    'line-height': $data.innerHeight() + "px"
                                });
                                $('.c1 > label, ', $data).css({
        ...
        $('.collapse').on('shown', function() {
            ...
            $('html,body').animate({
                scrollTop: sectiontopOffset - bulterheight
            }, 600);
        });
        ...
        $person.on('hover touchstart', function(e) {
            ...
            jQuery({
                val: parseInt($months.text())
            }).animate({
                val: $this.data('months')
            }, {
                duration: 500,
                easing: 'linear',
                step: function() {
        ...
        if ($('.elastik-configurator').length) {
            ...
            var EC = $('.elastik-configurator'),
                ...
                price = 0,
                startAt = parseInt( EC.data('start-at') ) || 0,
            ...
            EC.on('click', '.mobile-configurator .button', function() {
                var that = $(this),
                    ...
                    multiplier = EC.hasClass('xmas') ? 14.285714286 : 10,
                    topLimit = EC.hasClass('xmas') ? 7 : 10;

                if (operation === 'subtract' && startingGB > 0) {
                    startingGB--;

                    if( startingGB >= startAt ) {
                        price--;
                    }
                } else if (operation === 'add' && startingGB < topLimit) {
                    if (priceLocked) {
                ...
                mobileGB.css({
                    'min-width': startingGB * multiplier + '%',
                    'max-width': startingGB * multiplier + '%'
                });
        ...
        if ($.fn.select2 != undefined) {
            $("input.combobox").select2({
                ...
                formatAjaxError: function() {
                    return 'Iskanje ni vrnilo rezultatov';
                },
                formatLoadMore: function() {
                    return 'Nalagamo več rezultatov...';
                },
                formatSearching: function() {
                    return 'Iščemo...';
                },
                formatInputTooShort: function(term, minLength) {
                    return 'Prosimo vnesite vsaj ' + minLength + ' znak.';
                }
            });
        }
        ...
        if( $('.big-carousel').length ) {
            var bigCarousel = $('.big-carousel'),
                ...
                hiddenCarouselItems = $('.hidden-carousel-items');
            ...
            bigCarousel.on('click', '.carousel-btn', function(){
                var direction = $(this).data('direction');
                carousel.trigger('owl.' + direction);
            });
        }

        function filterItemsPerCategory(cat) {
            $('.hidden-carousel-items .item.' + cat).each(function(){
                var elem = $(this).parent().html();
                carousel.data('owlCarousel').addItem(elem);
                $(this).parent().remove();
            });

            $('.owl-carousel .item:not(.' + cat + ')').each(function(){
                var elem = $(this).parent(),
                    targetPos = elem.index();

                $(elem).clone().appendTo( hiddenCarouselItems );
                carousel.data('owlCarousel').removeItem(targetPos);
            });
        }

        filterItemsPerCategory('mobile');

        $('body').on('click', '.box-package-details', function(e){
            e.preventDefault();

            var that = $(e.currentTarget),
                moreDetails = that.find('.more-details'),
                moreDetailsHeight = moreDetails[0].scrollHeight,
                button = that.find('.open-more-details'),
                text = button.children('#button-text'),
                type = that.attr('data-type');

            that.toggleClass('closed');

            moreDetails.css('max-height', that.hasClass('closed') ? 0 : moreDetailsHeight);

            that.hasClass('closed') ? text.html('Več o paketu') : text.html('Skrij');

            filterItemsPerCategory(type);

            /** Toggle icons */
            that.find('i').toggleClass('ico-arrow-down-brown ico-arrow-up-brown');
        });

        $('.package-box-details').on('click', '.show-more', function(e) {
            e.preventDefault();

            var that = $(this),
                details = that.parents('.package-box-details'),
                subDetails = details.siblings('.box-package-sub-details'),
                subDetailsHeight = subDetails[0].scrollHeight;

            details.addClass('opened');

            subDetails.css('max-height', subDetailsHeight);
        });

        $('.box-package-sub-details').on('click', '.hide-details', function(e) {
            e.preventDefault();

            var that = $(this),
                subDetails = that.parents('.box-package-sub-details'),
                details = subDetails.siblings('.package-box-details');

            subDetails.css('max-height', 0);
